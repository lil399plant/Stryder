...
import type { TrainingPlan, TrainingSession, Caregiver } from "@/lib/types";
...
export function PlanDetail({ plan, onBack }: { plan: TrainingPlan; onBack: () => void }) {
  ...
  return (
    <div>
      ...
      <h1 className="text-[20px] font-semibold leading-tight">{plan.name}</h1>
      <p className="mt-1.5 text-[14px] leading-snug">{plan.goal}</p>
      <p className="mt-1 text-[12.5px] leading-snug text-muted-foreground">{plan.whyItMatters}</p>
